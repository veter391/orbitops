...
- **Probability of collision** — two implementations. A fast first-order 2D-Gaussian estimate (`agents/conjunction.ts`), `Pc ≈ exp(−d²/2σ²)·(1 − exp(−R²/2σ²))` (the leading Chan/Alfano term, for a single isotropic σ); **and the high-fidelity full-covariance method** (`conjunction/pc2d.ts`) — the **Foster-1992 / NASA CARA `Pc2D_Foster`** approach: project both objects' 3×3 position covariances into the encounter plane, combine, and integrate the 2D Gaussian over the hard-body disk (analytic erf inner integral + adaptive-Simpson outer quadrature). It is **validated against NASA CARA's own Omitron reference vector to a relative error of ~3×10⁻⁶** (≈260× tighter than NASA's 1×10⁻³ test tolerance) and independently cross-checked by a second, from-scratch quadrature. The full method captures anisotropic covariance (e.g. σ 0.8 vs 3.4 km) that the single-σ estimate cannot. Both band to operator thresholds (`Pc ≥ 1e-3` critical, `1e-4` warning, `1e-5` watch).
