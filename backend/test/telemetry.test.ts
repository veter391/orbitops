...
test('ingest stores a batch and does not touch the audit log', async () => {
  const auditBefore = await app.audit.count();
  ...
  assert.equal(await app.audit.count(), auditBefore); // telemetry is not audited
});
...
test('queryBucketed downsamples into fixed-width buckets', async () => {
  ...
  // readings at +0/+10s fall in bucket 0; +70s in bucket 1 → two buckets
  assert.equal(series.length, 2);
  const totalN = series.reduce((s, b) => s + b.n, 0);
  assert.equal(totalN, 3);
});

test('latest returns one reading per metric', async () => {
  const res = await app.inject({ method: 'GET', url: '/v1/telemetry/latest?satelliteId=oo1-01' });
  assert.equal(res.statusCode, 200);
  const points = (res.json() as { points: { metric: string; value: number }[] }).points;
  const metrics = points.map((p) => p.metric).sort();
  assert.deepEqual(metrics, ['battery_v', 'cpu_c']);
  const batt = points.find((p) => p.metric === 'battery_v')!;
  assert.equal(batt.value, 26.6); // newest battery reading (at +70s)
});

test('rejects an empty batch and a non-finite value', async () => {
  const empty = await app.inject({ method: 'POST', url: '/v1/telemetry', payload: { readings: [] } });
